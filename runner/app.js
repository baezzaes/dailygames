...
function drawHUD(){ctx.save();ctx.fillStyle="rgba(0,0,0,.5)";ctx.fillRect(0,0,canvas.width,34);ctx.fillStyle="rgba(255,255,255,.9)";ctx.font="700 13px system-ui";ctx.textBaseline="middle";ctx.textAlign="left";ctx.fillText(`점수 ${game.score}`,10,17);ctx.textAlign="right";ctx.fillText(`${game.time.toFixed(1)}s`,canvas.width-10,17);ctx.restore();}
function draw(){drawBG();for(const it of game.items)drawItem(it);drawPlayer();drawOverlay();drawHUD();}
